export type CatalogImportSourceType = 'photo' | 'old_quote' | 'pdf' | 'spreadsheet';
...
export interface CatalogArticleDraft {
  description: string;
  unitPrice: number;
  vatRate: number;
  unit?: string;
...
export function sanitizeCatalogText(value: unknown, max = 220): string {
  return String(value || '')
    .replace(/[\u0000-\u001F\u007F]/g, ' ')
    .replace(/\s+/g, ' ')
    .trim()
...
export function parseCatalogNumber(value: unknown, fallback = 0): number {
...
export function parseCatalogUnit(value: unknown): string {
...
export function clampVatRate(value: unknown): number {
...
export function previewItemToArticle(
  item: CatalogImportPreviewItem,
  source: CatalogArticleDraft['source'] = 'ai_import',
): CatalogArticleDraft {
  return {
    description: item.description || item.name,
    unitPrice: roundMoney(item.priceHT),
    vatRate: clampVatRate(item.vatRate),
